...
3. Curate the chat (see Chat). Ask the whole remaining frontier. Wait.

A claim whose answer depends on another claim still open in this round belongs to a later round.

## Round format

Skip the preamble. Use the smallest visual that makes the point, with brief prose beside it: Mermaid, trees, or one HTML file. Point at file anchors; do not paste the findings file.
...
Put files where the repo already keeps notes; if there is none, use `findings/` and say the path.
...
- **`view-{description}.html`** — a round's *view* when Mermaid is not enough. Disposable. Do not HTML-wrap a plot the notebook already shows unless the plot needs a curated frame.

Chat is the round protocol. Files are the evidence locker.

## Done

The frontier is empty: every in-scope claim is settled or explicitly bounded-unknown, each with documentary evidence. Stop. Wait for the user to confirm the shared reading. Do not start a design interview off the findings.
